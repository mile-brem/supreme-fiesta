...
library(qs2)
library(scCustomize)
merged_local_int <- qs_read("/lisc/data/work/tessmar_raible/regeneration/_2025_neurogenesis/251022_regeneration_integration/03_downsample_latest/merged_local_int.qs2")
subcluster_neuronal_alt <- qs_read("/lisc/data/work/tessmar_raible/regeneration/_2025_neurogenesis/251022_regeneration_integration/03_downsample_latest/subcluster_neuronal_alt.qs2")
```
...
The first thing for RNA velocity however is I need to calcualte the spliced and unspliced RNA. This is done on the command line and so fuck doing this in R again and we'll move to python.

Let's convert this do anndata gain. Before that though I need to add the velocyto output.
...
```{r}
merged_local_int_ann <- as.anndata(
    x = merged_local_int,
    file_path = "/lisc/data/work/tessmar_raible/regeneration/_2025_neurogenesis/251022_regeneration_integration/03_downsample_latest/",
    file_name = ("merged_local_int_ann.h5ad"),
    main_layer = "data",
    other_layers = NULL
  )
```
...
```{r}
# Pick the matrix you want — usually EigenVectors
